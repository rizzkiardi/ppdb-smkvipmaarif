...
<div class="px-6 py-12 max-w-screen-xl xl:mx-auto">
    <h1 class="font-semibold">Frequently Ask Questions (FAQ)</h1>
    <p class="text-sm">Halaman ini untuk membuat Pertanyaan dan Jawaban, yang akan di tampilkan di Landing page.</p>
    <div class="my-5">
        ...
        <table data-tab-for="order" data-page="active" class="w-full min-w-[540px] text-sm text-left border-t border-slate-100 rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-2 py-3">Pertanyaan</th>
                    <th scope="col" class="px-2 py-3">Jawaban</th>
            ...
            <tbody>
                @foreach($faqs as $faq)
                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-2 py-4">{{ $faq->question }}</td>
                        <td class="px-2 py-4">{{ $faq->answer }}</td>
                        <td class="px-2 py-4 flex gap-2 justify-center items-center ">
                            <a class="bg-green-100  text-green-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300" href="{{ route('faq.edit', $faq->id) }}">
                            ...
                            <button data-modal-target="popup-modal" data-modal-toggle="popup-modal" data-item-name="" data-url="{{ route('faq.destroy', $faq->id) }}" onclick="confirmDelete('{{ route('faq.destroy', $faq->id) }}')" type="button" class="bg-pink-100 text-pink-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-pink-900 dark:text-pink-300">
                                <div class="flex items-center gap-2">
